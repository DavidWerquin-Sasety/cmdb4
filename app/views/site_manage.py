...
def create_access(client_id,site_id):
    ...
    if form.validate_on_submit():
        acc=InternetAccess(site=site); form.populate_obj(acc); acc.type_id=None if form.type_id.data==0 else form.type_id.data
        db.session.add(acc); db.session.commit()
        if request.headers.get("HX-Request"): return render_row(acc)
        return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    if request.headers.get("HX-Request"):
        return render_template("sites/asset_form_row.html", form=form, colspan=COLSPAN["access"], back_url=url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
...
def edit_access(client_id,site_id,aid):
    acc=InternetAccess.query.get_or_404(aid); site=acc.site
    form=InternetAccessForm(obj=acc); populate_choices(site,form)
    if form.validate_on_submit():
        form.populate_obj(acc); acc.type_id=None if form.type_id.data==0 else form.type_id.data
        db.session.commit()
        if request.headers.get("HX-Request"): return render_row(acc)
        return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    if request.headers.get("HX-Request"):
...
def create_license(client_id,site_id):
    site=Site.query.get_or_404(site_id)
    if site.licenses: flash("Une seule licence","danger");return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    form=SaseLicenseForm(); populate_choices(site,form)
    if form.validate_on_submit():
        lic=SaseLicense(site=site); form.populate_obj(lic); lic.type_id=None if form.type_id.data==0 else form.type_id.data
        db.session.add(lic); db.session.commit()
        if request.headers.get("HX-Request"): return render_row(lic)
        return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    if request.headers.get("HX-Request"):
        return render_template("sites/asset_form_row.html",form=form,colspan=COLSPAN["license"], back_url=url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
...
def edit_license(client_id,site_id,lid):
    lic=SaseLicense.query.get_or_404(lid); site=lic.site
    form=SaseLicenseForm(obj=lic); populate_choices(site,form)
    if form.validate_on_submit():
        form.populate_obj(lic); lic.type_id=None if form.type_id.data==0 else form.type_id.data
        db.session.commit()
        if request.headers.get("HX-Request"): return render_row(lic)
        return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    if request.headers.get("HX-Request"):
...
def create_equipment(client_id,site_id):
    ...
    if form.validate_on_submit():
        if not equipment_ok(site, form.type_id.data if form.type_id.data!=0 else None):
            return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
        eq=SaseEquipment(site=site); form.populate_obj(eq); eq.type_id=None if form.type_id.data==0 else form.type_id.data
        db.session.add(eq); db.session.commit()
        if request.headers.get("HX-Request"): return render_row(eq)
        return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    if request.headers.get("HX-Request"):
        return render_template("sites/asset_form_row.html",form=form,colspan=COLSPAN["equipment"], back_url=url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
...
def edit_equipment(client_id,site_id,eqid):
    eq=SaseEquipment.query.get_or_404(eqid); site=eq.site
    form=SaseEquipmentForm(obj=eq); populate_choices(site,form)
    if form.validate_on_submit():
        if not equipment_ok(site, form.type_id.data if form.type_id.data!=0 else None):
            return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
        form.populate_obj(eq); eq.type_id=None if form.type_id.data==0 else form.type_id.data
        db.session.commit()
        if request.headers.get("HX-Request"): return render_row(eq)
        return redirect(url_for('site_manage.site_detail',client_id=client_id,site_id=site_id))
    if request.headers.get("HX-Request"):
